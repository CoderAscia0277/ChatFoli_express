...
const SubmitIcon = ({isRequesting,send = () => null}) => {
    ...
    if(isRequesting){
        ...
        return(
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" onClick={() =>  send()} className={`${Theme.IconSize} hover:scale-110 cursor-pointer`} color="#F8F9FA" fill="none">
...
const UserTextArea = ({ action = () => null, set_request_state = () => null, isRequesting}) => {
    ...
    return(
        <>
            ...
            <SubmitIcon send={() => submit_action()} isRequesting={isRequesting}/>
        </>
       
    );
};

const Bubble = ({scrollUp = () => null,value,response_type,set_request_state = () => null}) => {
    const Theme = useContext(ThemeContext);
...
const MessageScrollView = () => {
    ...
    const add_bubble = useCallback((text,response_type) => {
        const new_block = <Bubble key={chat_blocks.length} scrollUp={() => ScrollUp()} set_request_state = {(bool) => set_request_state(bool)} response_type={response_type} value={text}/>;
