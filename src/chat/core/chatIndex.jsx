...
const ChatIndex = () => {
    ...
    const IconRenderer = ({data = [{'url':''},{'onClick':'func'}]}) => {

        const {url,onClick} = data;

        return(
            <img onClick={() => onClick()} className="w-1/4 h-full rounded-lg hover:scale-105 cursor-pointer" alt="..." src={url} style={{aspectRatio:1/1, background:theme.light }}  />
        ...
        <div className="w-full h-12  flex flex-grow flex-row  justify-around px-4 py-2 items-center mb-2 gap-4">
            {
                ...
                <svg xmlns="http://www.w3.org/2000/svg" onClick={() => select_icon(0)}  fill="currentColor" className="bi bi-emoji-wink w-7 h-7 text-neutral-400 text-white hover:scale-110  hover:cursor-pointer " viewBox="0 0 16 16">
            ...
            }
            {
                ...
                <svg xmlns="http://www.w3.org/2000/svg"  fill="currentColor" className="bi bi-map-fill w-7 h-7  text-neutral-400 text-white hover:cursor-pointer " viewBox="0 0 16 16">
                 ...
                 <svg xmlns="http://www.w3.org/2000/svg" onClick={() => select_icon(1)}  fill="currentColor" className="bi bi-map  w-7 h-7 text-neutral-400 text-white hover:scale-110  hover:cursor-pointer " viewBox="0 0 16 16">
            ...
            }
            {
                ...
                <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" className="bi bi-briefcase-fill w-7 h-7 text-neutral-400 text-white hover:cursor-pointer " viewBox="0 0 16 16">
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" onClick={() => select_icon(2)}  fill="currentColor" className="bi bi-briefcase  w-7 h-7 text-neutral-400 text-white hover:scale-110  hover:cursor-pointer" viewBox="0 0 16 16">
            ...
            }
            {
                ...
                <svg xmlns="http://www.w3.org/2000/svg" onClick={() => select_icon(3)}  fill="currentColor" className="bi bi-chat-right-text w-7 h-7 text-neutral-400 text-white hover:scale-110  hover:cursor-pointer" viewBox="0 0 16 16">
    ...
    const ClickableChatStickers = ({value}) => {

        const IconData = [{'url':'...','onClick':() => console.log('Icon clicked')}
                        ,{'url':'./images/Foli.png','onClick':() => console.log('Icon clicked')}
                        ,{'url':'...','onClick':() => console.log('Icon clicked')}
        ]
            ...
            <div className=" w-full flex-grow flex  flex-col gap-4 pb-2 px-4 overflow-y-auto">
                {
                    IconData.map((item,key) => {
                        return  <div className="w-full  flex flex-row   items-center justify-around " key={key}>
                            {
                                IconData.map((item,key) => {
                                    return <IconRenderer data={item} key={key}/>
                                })
                            }
                        </div>
                    })
            ...
            <article className={`w-full absolute bottom-0  z-10 transition-all flex flex-col pt-4 px-2  ${!DialogueState ? '' : 'opacity-0 pointer-events-none'}  `} style={{backgroundColor:'rgba( 32, 32, 32, 0.20)',backdropFilter:'blur(1px)'}}>
                <div className="flex flex-row items-center justify-center gap-3 px-0 ">
                    <span className={`  ${isTextFieldFocus ? 'w-0 hidden' : 'w-max flex'}  flex-row justify-center items-center gap-4`}>
                        ...
                        <span className={`rounded-full   overflow-hidden scale-110 ${isTextFieldFocus ? 'p-0 w-0' : 'p-2'}`} style={{background:chat_menu_panel ? theme.light : theme.dark}}>
                            <svg xmlns="http://www.w3.org/2000/svg" onClick={() => set_chat_menu_panel(!chat_menu_panel)} fill="currentColor" className="bi bi-three-dots w-7 h-7 text-neutral-400 text-white hover:cursor-pointer  " viewBox="0 0 16 16">
                ...
                <div className={`${chat_menu_panel ? 'push_up flex' : chat_menu_panel === false ? 'pull_down' : ''} h-0 px-0 pt-2 overflow-hidden `}>
                    <div className="h-full w-full flex flex-col transition-all rounded-md  pt-4" style={{background:theme.dark}}>

                        <ClickableChatStickers/>
                        <ChatPanelIcons/>
