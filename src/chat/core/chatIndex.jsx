...
const ChatIndex = () => {
    ...
    const SubmitText = () => {
        ...
        setTimeout(()=>{
            if(ItemPicker > 50){
                DialogueBlocks = [...DialogueBlocks,<Dialogue target="char" name="Suzumi" value={'Testing testing'} key={DialogueBlocks.length}/>];
                setDialogueBlocks(DialogueBlocks);
            }else{
                DialogueBlocks = [...DialogueBlocks,<Dialogue target="nar"  value={'Narrator testing'} key={DialogueBlocks.length}/>];
                setDialogueBlocks(DialogueBlocks);
            }
        },500);
    }
    ...
    return(
        <section className="lg:w-2/6 md:w-2/5 w-full lg:3/4 md:3/4 h-full absolute xs:left-0  lg:top-0 md:top-0 bottom-0 bg-neutral-900 lg:rounded-xl md:rounded-xl  mt-0 flex flex-col py-2 ">
            ...
            <article className="w-full min-h-20  flex flex-row items-center justify-around px-2">
                ...
                <textarea onKeyDown={HandlePress} placeholder="Say something . . ." onFocus={() => setTogglingPanel(toggling_panel ? false : '')} onChange={HandleChange}  className="text-start bg-neutral-800 outline-0 lg:w-3/5 md:w-3/5 sm:w-3/4 w-1/2 py-2 text-white rounded-2xl flex flex-row items-center px-4 " style={{border: 'solid 1px rgb(67 67 67)',resize:'none'}}></textarea>
                
                { filled ? 
                    <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" onClick={() => SubmitText()} className="bi bi-arrow-up-square-fill w-10 h-10 text-white cursor-pointer" viewBox="0 0 16 16">
