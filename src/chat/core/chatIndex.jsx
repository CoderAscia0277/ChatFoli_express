        ...
        <section className="lg:w-2/6 md:w-2/5 w-full lg:3/4 md:3/4 h-full absolute xs:left-0  lg:top-0 md:top-0 bottom-0  lg:rounded-xl md:rounded-xl  mt-0 flex flex-col pt-2 " style={{background:'url(./images/classroom_bg.jpg) center/cover no-repeat'}}>
            <article className="w-full min-h-16 flex flex-row items-center px-2">
                ...
                <div className="flex-grow h-full flex flex-row justify-center items-center">
                    <span className="font-sans font-bold min-w-20 min-h-10 text-white rounded-2xl flex flex-row items-center px-4" style={{border:'solid 1px rgb(67 67 67)',background:theme.dark}}>
                        Moe Moe Paradise!
                    </span>
                </div>
            ...
            <article className={`w-full absolute bottom-0 z-10 transition-all min-h-20 flex flex-col  px-4 ${DialogueState ? '' : 'opacity-0 pointer-events-none'}  `} style={{backgroundColor:'rgba( 32, 32, 32, 0.20)',backdropFilter:'blur(1px)'}}>
                <div className="flex flex-row items-center justify- gap-4">
                    ...
                    {/* <span className={`rounded-full  p-2`} style={{backgroundColor:'rgb(61 61 61)'}} >
                        ...
                        <textarea onKeyDown={HandlePress} onBlur={()=>setTextFieldFocus(false)} placeholder="Say something . . ." onFocus={() => {setEmojiToggle(false); setTextFieldFocus(true)}} onChange={HandleChange}  className={`text-start bg-transparent transition-all outline-0 flex-grow py-2 text-white  flex ${isTextFieldFocus ? 'w-full' : 'w-3/4'} items-center px-2 `} style={{resize:'none'}}></textarea>
                ...
                <div className={`${EmojiSticker ? 'h-48 flex' : 'h-0 hidden'} border transition-all `}>
