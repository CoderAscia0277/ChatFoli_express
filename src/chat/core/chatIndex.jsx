        ...
        <section className="lg:w-2/6 md:w-2/5 w-full lg:3/4 md:3/4 h-full absolute xs:left-0  lg:top-0 md:top-0 bottom-0 bg-neutral-900 lg:rounded-xl md:rounded-xl  mt-0 flex flex-col">
            <article className="w-full min-h-16 flex flex-row items-center px-2">
                ...
                <div className="flex-grow h-full flex flex-row justify-center items-center">
                    <span className="font-sans font-bold bg-neutral-800 min-w-20 min-h-10 text-white rounded-2xl flex flex-row items-center px-4">
                        Neko Paradise!
                    </span>
                </div>
            ...
            <article className="w-full flex-grow flex flex-col overflow-y-scroll container  px-4 pt-4">
                <Dialogue/>
                <Dialogue/>
                <Dialogue/>
                <Dialogue/>
            ...
            <article className="w-full absolute z-10 bottom-20 bg-neutral-800 " style={{animation: toggling_panel ? 'push_up 250ms ease-in-out forwards' : toggling_panel === false ? 'pull_down 250ms ease-in-out forwards' : '' }}>

            </article>
            <article className="w-full min-h-20  flex flex-row items-center justify-around px-2">
                <svg xmlns="http://www.w3.org/2000/svg" onClick={() => setTogglingPanel(!toggling_panel)}  fill="currentColor" className="bi bi-lightbulb-fill w-6 h-6 text-white hover:cursor-pointer hover:scale-110" viewBox="0 0 16 16">
                    <path d="M2 6a6 6 0 1 1 10.174 4.31c-.203.196-.359.4-.453.619l-.762 1.769A.5.5 0 0 1 10.5 13h-5a.5.5 0 0 1-.46-.302l-.761-1.77a2 2 0 0 0-.453-.618A5.98 5.98 0 0 1 2 6m3 8.5a.5.5 0 0 1 .5-.5h5a.5.5 0 0 1 0 1l-.224.447a1 1 0 0 1-.894.553H6.618a1 1 0 0 1-.894-.553L5.5 15a.5.5 0 0 1-.5-.5"/>
                </svg>
                <svg xmlns="http://www.w3.org/2000/svg" onClick={() => setTogglingPanel(!toggling_panel)} fill="currentColor" className="bi bi-emoji-wink-fill w-6 h-6 text-white hover:cursor-pointer hover:scale-110 " viewBox="0 0 16 16">
                ...
                <textarea placeholder="Say something . . ." onFocus={() => setTogglingPanel(toggling_panel ? false : '')} onChange={HandleChange}  className="text-start bg-neutral-800 lg:w-3/5 md:w-3/5 sm:w-3/4 w-1/2 py-2 text-white rounded-2xl flex flex-row items-center px-4 " style={{resize:'none'}}></textarea>
