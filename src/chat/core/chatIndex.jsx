...
const ChatIndex = () => {
    const [emoji_btn,setEmoji] = useState();
    return(
        <section className="lg:w-2/6 md:w-2/5 w-full lg:3/4 md:3/4 h-full absolute xs:left-0  lg:top-0 md:top-0 bottom-0 bg-neutral-900 lg:rounded-xl md:rounded-xl  mt-0 flex flex-col">
            ...
            <article className="w-full border" style={{animation: emoji_btn ? 'push_up 500ms ease-in-out forwards' : emoji_btn === false ? 'pull_down 500ms ease-in-out forwards' : '' }}>
            ...
            <article className="w-full min-h-20  flex flex-row items-center justify-around px-2">
                ...
                <svg xmlns="http://www.w3.org/2000/svg" onClick={() => setEmoji(!emoji_btn)} fill="currentColor" className="bi bi-emoji-wink-fill w-6 h-6 text-white hover:cursor-pointer hover:scale-110 " viewBox="0 0 16 16">
                ...
                <textarea  className="bg-neutral-800 lg:w-3/5 md:w-3/5 sm:w-3/4 w-1/2 min-h-10 text-white rounded-2xl flex flex-row items-center px-4 " style={{resize:'none'}}></textarea>
