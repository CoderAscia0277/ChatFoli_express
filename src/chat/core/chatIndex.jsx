...
const ChatIndex = () => {
    ...
    return(
        <section className="lg:w-2/6 md:w-2/5 w-full lg:3/4 md:3/4 h-full absolute xs:left-0  lg:top-0 md:top-0 bottom-0  lg:rounded-xl md:rounded-xl  mt-0 flex flex-col pt-2 " style={{background:'url(./images/classroom_bg.jpg) center/cover no-repeat'}}>
            ...
            <article className={`w-full overflow-hidden transition-all min-h-20 flex flex-row items-center justify- gap-4 px-2 ${DialogueState ? '' : 'opacity-0 pointer-events-none'}  `} style={{backgroundColor:'rgba( 32, 32, 32, 0.20)'}}>
                <span className={`${isTextFieldFocus ? 'w-0' : 'w-1/6'} transition-all flex flex-row justify-center items-center gap-4`}>
