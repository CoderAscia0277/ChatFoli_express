...
const LoginPage = () => {
    ...
    return(
        <section className="lg:h-3/4 lg:w-1/3 w-screen h-screen  border-neutral-700  rounded-lg flex flex-col justify-center">
            ...
            <form onKeyDown={e => e.key === 'Enter' && text_box.current.value && pass_box.current.value ? Validate({username:text_box.current,password:pass_box.current}) : null} className=" flex flex-col justify-evenly items-center h-1/2 text-neutral-300 px-4">
                ...
                <input type="button" onClick={() => Validate({username:text_box.current,password:pass_box.current})} value="Submit" className="w-1/4 p-2  rounded-md cursor-pointer hover:scale-105" style={{background:Theme.BlueGradient90}}/>
            </form>
